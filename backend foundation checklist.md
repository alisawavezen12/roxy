...
- [ ] Проверить, что backend-контейнер собирается
- [ ] Настроить локальные переменные окружения
- [ ] Убедиться, что данные PostgreSQL сохраняются в volume
- [ ] Отдельно зафиксировать MinIO как будущую зависимость, но пока не подключать его к backend
