...
| HTTP CSRF gate | **Missing** | HTTP middleware не подключает CSRF-проверку: запрещённый CORS Origin не предотвращает исполнение обычного запроса. До cookie-authenticated unsafe operations выбрать Origin/Referer и/или token policy, включая missing-Origin; готовый Wisp `csrf_known_header_protection` существует, но его same-origin семантику нужно согласовать с разрешённым cross-origin frontend. |
...
**Blocking before business features:** для начала read-only разработки и подключения user-scoped handlers session lifecycle/cookie policy теперь готовы; перед cookie-authenticated writes остаётся подключить CSRF gate. Перед login/logout business endpoints нужно использовать `transport.session.issue`/`revoke`, а не создавать собственную cookie policy.
